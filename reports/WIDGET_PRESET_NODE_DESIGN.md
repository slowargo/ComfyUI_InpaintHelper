...
| 调试信息 | 节点上一块只读文本区，实时列出所有参数路径和命中情况；在右键菜单里切换显示或隐藏 |
...
用户新连上一个节点时，如果 `filter` 里还没有以这个节点标题开头的规则，就在末尾追加一行，
列出它当前所有可记录的参数：
...
调试文本区是否显示，由右键菜单里的开关控制（见 7.5），状态存在 `node.properties.showDebug`，
随 workflow 保存，默认显示。隐藏时把它设为 `hidden`，并重新计算节点高度。
...
4. 写入值，并调用 widget 的回调：
   ```js
   widget.value = value;
   widget.callback?.(value, app.canvas, targetNode);
   ```
   回调必须调，本仓库的 `LoadRecentImagePlusV1`、`FloatSelector` 都靠回调同步界面和内部状态
...
- `Show debug info` / `Hide debug info`：切换调试文本区的显示
...
节点上一块只读文本，通过右键菜单显示或隐藏，和高亮在同样的时机刷新。隐藏时不计算内容。内容是所有连着的节点上的全部参数，每行一个：
...
| 项 | 内容 |
|---|---|
| Vue 模式下按钮文字能否更新 | `WidgetButton.vue` 拿到的是否是原 widget 的响应式代理。决定 Vue 模式下高亮能不能做，实现前先测 |
| 状态行用什么 widget | 普通 text widget 在 Vue 模式下是可编辑的输入框，要找一个只读的显示方式 |
| 属性面板里的 `presets` | `hidden` 是否对右侧属性面板也有效，否则用户会看到整段 JSON |
| 输入槽加 label 的影响 | `matchesLegacyApi` 的判断用到了 label（1.47.6 的 `litegraphUtil.ts:282-284`，1.37.2 未核对），加 label 前验证一下 |
| 自动追加规则和撤销 | 连线触发的规则追加和连线本身是否在同一条撤销记录里。不在的话，撤销连线后那行规则会留下 |
| `control_after_generate` | 这个下拉框按现在的条件会被一起记录，于是 Apply 会连带改掉用户的随机策略。不记录的话，seed 写进去，下次运行又被随机掉，高亮也随之消失。需要决定是否默认排除 |
| Vue 模式下按钮回调没有参数 | `WidgetButton.vue` 调回调时不传事件，`canvas.prompt` 的输入框会出现在左上角。legacy 模式不受影响 |
